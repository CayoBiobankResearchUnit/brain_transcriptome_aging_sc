...
mus.meta = readRDS('checkpoints/musmus_metadata.rds')

# Ignore cells that have no reference labels
mus.meta = subset(mus.meta,class_label != '')

mac = cds.matrix[,rownames(mac.meta)]
hom = cds.matrix[,rownames(mus.meta)]

macmul.integrated = Matrix(mac,sparse=TRUE)
musmus.integrated = Matrix(hom,sparse=TRUE)

macmul.singler.integrated = SingleR(
	test=macmul.integrated,
	ref=musmus.integrated,
	labels=mus.meta$cell_type_accession_label,
	method='cluster',
	clusters=mac.meta$assigned_cell_cluster)
saveRDS(macmul.singler.integrated,file=paste0('checkpoints/',dataset,'_musmus_singleR_integrated.rds'))
...
musmus.class = unique(subset(mus.meta,select=c('cell_type_accession_label','class_label','subclass_label','cell_type_alias_label')))
...
macmul.integrated.hits = do.call(rbind,lapply(1:nrow(macmul.singler.integrated),function(i) {
 	...
 	these.scores = merge(these.scores,musmus.class,by='cell_type_accession_label')
...
saveRDS(macmul.integrated.hits,file=paste0('checkpoints/',dataset,'_musmus_singleR_integrated_results_matches.rds'))
