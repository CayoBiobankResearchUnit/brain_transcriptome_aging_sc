...
cds = readRDS(paste0('checkpoints/',dataset,'_macmul_musmus_integrated.rds'))

cds = RunPCA(object = cds, verbose = FALSE)

cds = RunUMAP(object = cds, dims = 1:30)
saveRDS(cds,file=paste0('checkpoints/',dataset,'_macmul_musmus_integrated_umap.rds'))
...
a.umap$dataset = 'mouse'
a.umap$dataset[!is.na(a.umap$assigned_cell_type)] = 'rhesus'

saveRDS(a.umap,file=paste0('checkpoints/',dataset,'_macmul_musmus_integrated_umap_only.rds'))
...
p = ggplot() +
	geom_point_rast(aes(UMAP_1,UMAP_2),data=subset(a.umap,dataset=='mouse'),color='#cccccc',alpha=0.05,size=0.01,show.legend=FALSE) +
...
ggsave(p,file=paste0('figures/mouse_macaque_clustering_assigned_cell_type.pdf'),useDingbats=FALSE,height=5,width=10)


p = ggplot() +
	geom_point_rast(aes(UMAP_1,UMAP_2,color=class_label),data=subset(a.umap,dataset=='mouse' & !is.na(class_label)),alpha=0.05,size=0.01,show.legend=TRUE) +
...
#	scale_color_manual(name='Cell type',values=cell.colors,drop=TRUE) +
	...
	guides(color = guide_legend(override.aes = list(size=2,alpha=1))) +
	xlab('UMAP 1') + ylab('UMAP 2')
ggsave(p,file=paste0('figures/mouse_macaque_clustering_cell_class.pdf'),useDingbats=FALSE,height=5,width=10)


p = ggplot() +
	geom_point_rast(aes(UMAP_1,UMAP_2,color=class2),data=subset(a.umap,dataset=='mouse' & !is.na(class2)),alpha=0.05,size=0.01,show.legend=TRUE) +
	geom_point_rast(aes(UMAP_1,UMAP_2),data=subset(a.umap,dataset=='rhesus'),color='#cccccc',alpha=0.05,size=0.01) +
	coord_equal() +
#	scale_color_manual(name='Cell type',values=cell.colors,drop=TRUE) +
	...
	guides(color = guide_legend(override.aes = list(size=2,alpha=1))) +
	xlab('UMAP 1') + ylab('UMAP 2')
ggsave(p,file=paste0('figures/mouse_macaque_clustering_cell_subclass.pdf'),useDingbats=FALSE,height=5,width=10)
